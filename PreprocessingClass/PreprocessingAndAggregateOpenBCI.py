...
class PreprocessingAndAggregateOpenBCI:
    ###############################################################################################################################################
    # PreprocessingAndAggregateOpenBCI class
    # 1. Filtering the raw data : Power supply filter(50 Hz) -> Muscle Filter (0.5 Hz) -> R-Peak filter (15-20 Hz) -> ECG range filter(0.67-5 Hz)
    #   1.1 Including the visualization of signal
    # 2. Extract Heart rate, R-Peak and R-R interval : Apply find peak library from scipy.signal.findpeaks
    # 3. Get estimated heart rate, rr_interval and grouping it by timestamp => also called "feature dataframe"
    # 4. Get time interval to let user know that what is possible interval to sliced the dataframe correctly
    # 5. Get interest interval from feature dataframe (Slicing dataframe)
    # 6. Seperate feature dataframe by using channel
    #
    ###############################################################################################################################################
    def __init__(self, filename, filemode=1, process_channel=['Channel 1', 'Channel 2']):
        # Initialization of attribute 
        self.filename = filename    # filename to load .csv file
        self.filemode = filemode    # filemode that use to control the source(0 = GUI, 1 = Python script)
        self.process_channel = process_channel    # process_channel to select which channel should be process
        self.ecg_data = pd.read_csv(self.filename, usecols=['Time', 'Timestamp'] + self.process_channel)    # Read the .csv file and take only the ['Time', 'Timestamp'] and process_channel columns
        
        # To let the user know what time can be the interest interval
        self.possible_start_time = None    # Contain the possible start time of the data
        self.possible_end_time = None    # Contain the possible end time of the data
        
        self.estimated_heart_rate_by_time_ibi_and_count_heart_rate_by_time = pd.DataFrame()    # Contain the feature that calculated from R-Peak detection
        self.ecg_feature_df_interest_interval = None    # Contain the feature that sliced into interest interval range
        self.aggregate_interest_interval = None    # Contain the aggregate data and can be use to find count heart in 1 minutes or less
        self.time_interval = None    # Time interval from start to end (set by user)
        self.ecg_feature_df_interest_interval_sep_channel = []    # Contain the dataframe feature seperate by channel


    def get_time_interval(self):
        ################################################################################################################
        # get_time_interval
        # 1. Get the possible time interval in data for ensure that your interest interval is able to reach
        #
        #   Input : Attributes name ecg_data using ['clock'] column of the first row and the last row
        #   Output : Possible time interval from start to end
        #
        ################################################################################################################
    ...
    def filter_function(self):
        ...
        for i in range(0, len(self.process_channel)):
            ...
            plt.figure()
            plt.plot(ecg_finale_data[2500:5000])
    ...
    def get_interest_interval_information(self, ecg_feature_df, start_time, end_time):
        ...
        # Calculate the difference of time from start to end of interest interval
        self.time_interval = end_time_timedelta - start_time_timedelta
        
        ### Slicing the dataframe in interest interval
        self.ecg_feature_df_interest_interval = ecg_feature_df.loc[start_time_object.time():end_time_object.time()]
        
        # Calculate aggregate('Sum', 'Mean' and 'Std' of data in interest interval
        ...
        # Return the 3 values => sliced dataframe, aggregate dataframe and time_interval
    ...
    def get_feature_df_interest_interval_each_channel(self):
        ####################################################################################################################
        # get_feature_df_interest_interval_each_channel
        # 1 .Seperate the ecg_feature_df_interest_interval each channel to each dataframe that can be use to plot easier
        #   **** Mention : Need to call get_interest_interval_information before call this function
        #   Input : Attribute names " ecg_feature_df_interest_interval" ==> ***** Data need to be splitted
        #   Output : Seperate ecg_feature_df_interest_interval by channel and have index as clock
        #
        ####################################################################################################################
        
        # Create an empty list to contain each seperate channel
        self.ecg_feature_df_interest_interval_each_channel = []
        
        # Seperate each channel in iteration and append to created list above
        for i in range(0, len(self.process_channel)):
            ...
            # Also drop the data that have no R-Peak in that time
            
            # Append seperated channel into list
            self.ecg_feature_df_interest_interval_sep_channel.append(temp)
            
        # return the seperated channel
